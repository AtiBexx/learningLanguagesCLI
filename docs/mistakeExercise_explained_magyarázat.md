...
**`realExit` flag** — Ez egy klasszikus C++ probléma: a `break` csak a **közvetlen szülő ciklust** állítja meg. Mivel itt egy `for (;;)` van egy `for` belsejében, a belső `break` önmagában nem lenne elég — a külső ciklus továbblépne a következő szóra. A `realExit` boolean ezt hidalja át: a külső ciklus feltételébe be van drótozva a `!realExit`.

**`ios::out | ios::trunc`** — A `|` itt **nem logikai VAGY**, hanem **bitenkénti VAGY** (bitwise OR). A flagek valójában egész számok, amelyeknek különböző bitjei vannak beállítva. A `|` összerakja őket egy értékbe, amelyből a könyvtár ki tudja olvasni mindkét kérést egyszerre. Ez C és C++ rendszerprogramozásban nagyon elterjedt mintázat.

**Duplikátum-probléma a `stillMistakes`-ben** — Ha egy szót háromszor is elront a felhasználó, háromszor kerül bele a listába. Ez nem bug, hanem a jelenlegi egyszerű implementáció korlátja — egy kifinomultabb verzióban ellenőrizni kellene, hogy az adott szó már benne van-e a listában.

**A `continue` az újrarajzoláshoz** — Ahelyett, hogy külön kódot írna a képernyő frissítéséhez, a súgó toggle egyszerűen `continue`-val ugrik a belső ciklus elejére, ahol úgyis le van kódolva a teljes képernyő kirajzolása. Ez elegáns megoldás: egy helyen van a megjelenítési logika.