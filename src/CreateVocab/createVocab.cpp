...
{
    // nyelvi fájlok
    const VocabWordsPairTotal &vwptSTR = vocabWordsPairTotalTranslations[static_cast<int>(currentLanguage)];
    const EnteringBack &entering_back = enteringBackTranslations[static_cast<int>(currentLanguage)];
    const CreateVocabFile &createVocabFile = createVocabFileTranslations[static_cast<int>(currentLanguage)];
    // Összegzés
    screenWipe();
    if (isExiting)
    {
    ...
    std::cout << entering_back.enteringBack << std::endl;
    std::cin.get();
}
...
void createVocab()
{
    //nyelvi fájlok
    const CreateVocabFile &createVocabFile = createVocabFileTranslations[static_cast<int>(currentLanguage)];
    const CreateVocab &createVocab = createVocabTranslations[static_cast<int>(currentLanguage)];
    ...
    for (;;)
    {
        ...
        std::getline(std::cin, currentWords.english);

        // Kilépés kezelése
        if (currentWords.english == "exit")
        {
        ...
        if (currentWords.english == "save") {
            outFile.flush(); // Kényszerített mentés lemezre
            wordsPairTotal(false);
            continue;
        }

        if (currentWords.english.empty()) continue;

        std::cout << createVocab.createVocab6;
        std::getline(std::cin, currentWords.hungarian);

        std::cout << createVocab.createVocab7;
        std::getline(std::cin, currentWords.pronunciation);

        // Fájlba mentés formázva
        outFile << trim(currentWords.english) << " -> " << trim(currentWords.hungarian);
